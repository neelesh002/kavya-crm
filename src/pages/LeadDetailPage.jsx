...
const TABS = [
  ...
  { key:'calls',    label:'Calls (7)' },
  { key:'messages', label:'Messages (12)' },
  { key:'emails',   label:'Emails (5)' },
  { key:'tasks',    label:'Tasks (3)' },
...
export default function LeadDetailPage() {
  ...
  const { id }   = useParams()
  const { leads, selectedLead, toast } = useApp()
  const [activeTab, setActiveTab] = useState('timeline')
  const [note, setNote] = useState('')

  const lead = selectedLead || leads.find(l => l.id === parseInt(id))
  if (!lead) return (
  ...
  const timeline = [
    { icon:'📞', bg:'var(--teal-dim)',   title:'Call Made',  note:'4min 28sec · Completed', time:'Today, 10:35 AM' },
    { icon:'💬', bg:'var(--orange-dim)', title:'WhatsApp Sent', note:'Delivered', time:'Today, 10:10 AM', msg:'Hi! Please find the proposal attached. Looking forward to your feedback!' },
    { icon:'✉️', bg:'var(--blue-dim)',   title:'Email: Follow-up Proposal — CRM Suite', time:'Yesterday, 04:20 PM' },
    { icon:'✅', bg:'var(--purple-dim)', title:'Task Created: Demo call scheduled',  time:'Yesterday, 11:00 AM' },
    { icon:'📝', bg:'var(--green-dim)',  title:'Status changed: NEW → CONTACTED',   time:'Mar 1, 03:00 PM' },
  ]
    ...
    <div className="animate-fadeup">
      ...
      <div className="page-header">
        ...
        <div className="page-actions">
          ...
          <button className="btn btn-outline"  onClick={() => toast('✉️ Email opened','info')}>✉️ Email</button>
          <button className="btn btn-primary"  onClick={() => navigate('/leads')}>✏️ Edit Lead</button>
        </div>
      </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <Card>
            ...
            {[
              ...
              ['Deal Value',  <span className="fw-700 text-green">₹{(lead.deal||0).toLocaleString()}</span>],
          ...
          <Card>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
              {[['7','Calls','var(--teal)'],['12','Messages','var(--orange)'],['5','Emails','#3b82f6'],['3','Tasks','#7c3aed']].map(([v,l,c]) => (
        ...
        <Card>
          ...
          {activeTab === 'calls' && (
            <div className="overflow-x-auto">
              <table className="data-table">
                ...
                <tbody>
                  {CALL_LOGS.filter(c => c.leadId === lead.id).map(c => (
                    <tr key={c.id}>
                  ...
                  {CALL_LOGS.filter(c => c.leadId === lead.id).length === 0 && (
          ...
          {activeTab === 'messages' && (
            <div className="flex flex-col gap-3">
              {MESSAGES_DATA.filter(m => m.leadId === lead.id).map(m => (
              ...
              {MESSAGES_DATA.filter(m => m.leadId === lead.id).length === 0 && (
          ...
          {activeTab === 'emails' && (
            <div className="overflow-x-auto">
              <table className="data-table">
                ...
                <tbody>
                  <tr><td>Follow-up Proposal — CRM Suite</td><td className="font-mono text-sm">Mar 5, 4:20 PM</td><td><StatusBadge status="COMPLETED" /></td></tr>
                  <tr><td>Meeting Confirmation — Mar 6</td><td className="font-mono text-sm">Mar 4, 10:00 AM</td><td><StatusBadge status="COMPLETED" /></td></tr>
                </tbody>
              </table>
            </div>
          )}
          ...
          {activeTab === 'tasks' && (
            <div className="flex flex-col gap-3">
              {[{ title:'Demo call scheduled', due:'Today', priority:'URGENT' }].map((t,i) => (
                <div key={i} style={{ background:'var(--bg0)', border:'1px solid var(--border)', borderRadius:'var(--r-sm)', padding:'12px 14px', display:'flex', alignItems:'center', gap:12 }}>
          ...
          {activeTab === 'notes' && (
            <div>
              ...
              <div style={{ background:'var(--bg0)', borderRadius:'var(--r)', padding:'13px 15px', border:'1px solid var(--border)' }}>
                ...
                <p style={{ fontSize:13.5, fontWeight:500, lineHeight:1.6 }}>Lead is very interested in the Enterprise plan. Has budget approval for ₹2.5L. Decision expected by end of March.</p>
              </div>
            </div>
          )}
        </Card>
